...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/f1_frozen.json"))
    ...
    outputs = [evaluate_session(pipeline, session, args.mode) for session in _sessions(args.sessions, store)]
